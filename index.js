...
const BOT_URL = "https://wakala-bot.onrender.com";
...
async function handleMessage(senderJid, messageText) {
  ...
  if (textLower.startsWith("lipa ")) {
    ...
    if (!isNaN(amt)) {
      ...
      if (tier) {
        ...
        await sendMessage(senderJid, "Kiasi hicho hakipo kwenye mfumo.");
      }
    }
    return;
  }
  ...
  if (session.step === "name") {
    if (!["alfany","alex"].includes(textLower)) {
      await sendMessage(senderJid, "Tafadhali andika:\n*Alfany* au *Alex*");
      return;
    }
  ...
  if (session.step.startsWith("float_")) {
    ...
    if (idx + 1 < FLOAT_FIELDS.length) {
      ...
      saveSessions(sessions);
      await sendMessage(senderJid,
        `✅ *${FLOAT_FIELDS[idx].label}: ${fmt(val)}*\n\nSasa *${FLOAT_FIELDS[idx+1].label}* (TZS)\n_(Andika 0 kama hakuna)_`
      );
    } else {
      session.step = "lipa_fees";
      saveSessions(sessions);
      await sendMessage(senderJid,
        "✅ Float zote zimesajiliwa!\n\n" +
        "Sasa tupe *huduma za lipa namba* ulizokusanya.\n\n" +
...
app.post('/webhook', async (req, res) => {
  ...
  try {
    ...
    if (!senderNumber.includes(ALLOWED_NUMBER)) return;
